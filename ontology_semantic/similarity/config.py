...
# El archivo vive en ``<root>/config/similarity_weights.yaml``.
...
class StructuralAttribute:
    ...
    Attributes:
        name: nombre de la property (en Neo4j) o pseudo-property
            (ej. ``categoria_id`` que se obtiene via join).
        type: ``"categorical"`` o ``"boolean"`` (booleanos se tratan como
            categoricos binarios en Gower).
        weight: peso INTERNO del scorer estructural. Suma con los pesos
            de los otros atributos antes de promediar.
    """
...
class SimilarityConfig:
    ...
    Attributes:
        weights: Dict con keys lexical/structural/behavioral/trend.
        threshold_default: Threshold por defecto para considerar similar.
        top_k_default: Cantidad de similares por SKU en precompute.
        embedding_model_name: Nombre del sentence-transformer.
        embedding_dim: Dimension del vector (para validar matrices).
        text_template: Plantilla con placeholders {nombre}, {nombre_corto},
            {categoria}, {grupo}, {pais_origen}.
        structural_attributes: Lista de :class:`StructuralAttribute`.
        behavioral_available: Si la senal mensual esta integrada.
        trend_available: Si hay TrendSignals en la ABox.
    """
...
def load_config(path: Path | str | None = None) -> SimilarityConfig:
    ...
    Args:
        path: Ruta al YAML. Si es ``None`` usa
            :data:`DEFAULT_CONFIG_PATH` (= ``<root>/config/similarity_weights.yaml``).

    Returns:
        Un :class:`SimilarityConfig` listo para inyectar en
        :class:`SimilarityEngine`.

    Raises:
        FileNotFoundError: si el archivo no existe.
        ValueError: si falta una clave requerida o un peso es negativo.
