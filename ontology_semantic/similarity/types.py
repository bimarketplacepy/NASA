...
class SimilarityResult:
    ...
    Attributes:
        sku_a: SKU de origen (en una arista ``:SIMILAR_A`` que se persiste,
            por convencion ``sku_a < sku_b`` lexicograficamente).
        sku_b: SKU comparado.
        score_lexical: Cosine sobre embeddings de texto. En [0, 1].
        score_structural: Distancia de Gower invertida. En [0, 1].
        score_behavioral: Cosine de ventas mensuales. En [0, 1].
            ``0.0`` si ``behavioral_flag`` indica missing.
        score_trend: Similitud lexica con TrendSignals. En [0, 1].
            ``0.0`` si no hay senales de tendencia disponibles.
        score_total: Composite ponderado segun pesos del YAML.
            Renormalizado cuando hay scorers missing.
        confidence: ``[0, 1]`` proxy de confianza, suma de pesos efectivamente
            usados. ``1.0`` si los 4 scorers aportaron, menos si alguno fue
            missing (importante para el dispatcher: bajo confidence debe
            triggerear fallback de cold-start).
        behavioral_flag: Estado de la senal comportamental para este par.
        flags: Otras flags textuales (ej. "trend_no_signal", "structural_partial").
            Lista vacia por default.
    """
    ...
    def __post_init__(self) -> None:
        # Validaciones soft (no rompemos si vienen ligeramente fuera de [0,1]
        # por errores de redondeo en numpy, pero sí si el calculo dio nonsense).
        for label, val in [
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        Convierte el Enum a su valor string. Las tuplas a listas. Listo
        para meter como properties de una arista ``:SIMILAR_A`` con
        ``UNWIND`` + ``CREATE r SET r += $props``.
        """
        return {
            "sku_a": self.sku_a,
            "sku_b": self.sku_b,
